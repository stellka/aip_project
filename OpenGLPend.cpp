...
{   
   ...
   EditA0 = CreateWindowEx( WS_EX_CLIENTEDGE, TEXT("Edit"), TEXT("0.3"),
                        ES_AUTOHSCROLL | ES_AUTOVSCROLL | WS_CHILD | WS_VISIBLE ,
...
void draw() 
{
	...
	const double g = 9.8f ;
	double w = sqrt(g/Pend_Len) ;
	
	double Am = Pend_A0+Pend_V0*Pend_V0/2.0f ;
	double Vm = sqrt(2.0f*Pend_A0+Pend_V0*Pend_V0) ;
		
	double a = Am*sin(w*t) ;
	double v = Vm*cos(w*t) ;

	energy_e.push_back(Pend_A0+Pend_V0*Pend_V0/2.0f-v*v/2) ;
	energy_k.push_back(v*v/2) ;
    ...
    glBegin (GL_TRIANGLES);
        ...
        glVertex3f( x1, y1, 0 );
		glColor3f(  0, 0.75, 0 );     
        glVertex3f( x1+dy*sin(a)-dx*cos(a), y1+dy*cos(a)+dx*sin(a), 0 );        
		glVertex3f( x1+dy*sin(a)+dx*cos(a), y1+dy*cos(a)-dx*sin(a), 0 );        
    glEnd();
	...
	glBegin (GL_TRIANGLES);
		...
		glColor3f(  0,0, 0.75 );     
        glVertex3f( x1+dy*sin(a)-dx*cos(a), y1+dy*cos(a)+dx*sin(a), 0 );        
		glVertex3f( x1+dy*sin(a)-dx*cos(a), y1+dy*cos(a)+dx*sin(a), 1 );         
    glEnd();
